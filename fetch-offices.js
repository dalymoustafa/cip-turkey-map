...
function extractCity(address) {
  ...
  for (let i = parts.length - 2; i >= 0; i--) {
    ...
    if (/^\d+$/.test(part)) continue;
    if (/\b(street|road|avenue|boulevard|floor|level|suite|tower|plaza|bldg|place|utca|district|avenida|rua|heights|zayed|cd|no|mescit|beyoglu)\b/i.test(part)) continue;
...
async function main() {
  ...
  const record = allRecords.find(r =>
    r.fields["Name"] && r.fields["Name"].toLowerCase().includes("cip turkey")
  );

  if (!record) { console.error("Could not find CIP Turkey!"); process.exit(1); }
...
<head>
  ...
  <title>CIP Turkey — Office Locations</title>
